...
public:
	MapLayer(const tmx::Map& map, std::size_t idx)
	{
		...
		if (map.getOrientation() == tmx::Orientation::Orthogonal && idx < layers.size() && layers[idx]->getType() == tmx::Layer::Type::Tile)
		{
...
private:
	...
	public:
			...
			m_animTiles(animTiles)
		{
			...
			const tmx::Vector2i& offset = layer.getOffset();
	...
	void createChunks(const tmx::Map& map, const tmx::TileLayer& layer)
	{
		...
		for (auto y = 0u; y < m_chunkCount.y; ++y)
		{
			...
			for (auto x = 0u; x < m_chunkCount.x; ++x)
			{
				...
				m_chunks.emplace_back(std::make_unique<Chunk>(layer, usedTileSets, sf::Vector2f(x * m_chunkSize.x, y * m_chunkSize.y), tileCount, tileSize, map.getTileCount().x, m_textureResource, map.getAnimatedTiles()));
			}
		}
	}
